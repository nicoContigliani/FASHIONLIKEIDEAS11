const stadisticsModel = require('./stadisticsModel')
const stadisticsDto = require('./stadisticsDto')
...
const getlikeAll = async (req, res) => {
    try {
        const postsdata = await stadisticsModel.getStadisticsT();
        console.log(postsdata)
        const postRow = await stadisticsDto.singles(postsdata)
        res.status(200).json(
            {
                data: postRow,
                status: 200
...
const getnotlikeAll = async (req, res) => {
    try {
        const postsdata = await stadisticsModel.getStadisticsF();
        const postRow = await stadisticsDto.singles(postsdata)
        res.status(200).json(
            {
                data: postRow,
